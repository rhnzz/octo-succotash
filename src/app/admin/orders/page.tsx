...
export default function AdminOrdersPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [purchases, sales] = await Promise.all([
        getMyPurchases(accessToken, params),
        getMySales(accessToken, params),
      ]);
      const merged = deduplicateOrders([...purchases.data, ...sales.data]);
      const filtered = statusFilter === 'ALL'
        ? merged
        : merged.filter((o) => o.status === statusFilter);
      setOrders(filtered);
      setTotalItems(Math.max(purchases.pagination.total_items, sales.pagination.total_items));
      setTotalPages(Math.max(purchases.pagination.total_pages, sales.pagination.total_pages));
    } catch (err) {
      if (isApiError(err)) {
  ...
  async function handleCancel() {
    ...
    try {
      await cancelOrder(accessToken, cancelModal.orderId, cancelReason);
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 space-y-6">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={statusFilter !== 'ALL' ? 'Tidak ada pesanan dengan status ini' : 'Belum ada transaksi di platform'}
          />
        ) : (
          <div className="space-y-3">
            {orders.map((order) => (
              <OrderCard key={order.order_id} order={order} viewAs="ADMIN" onAction={handleOrderAction} />
            ))}
          </div>
        )}
